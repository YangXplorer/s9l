...
- **Results（右上）**：查询/预览结果表格。
  - `/` **全字段模糊过滤行**（跨所有列、大小写不敏感的**子序列**匹配，如 `ae` 命中 `alice`；状态栏 `filtered M/N`）。
  - `f` **按选中列过滤**（仅当前列模糊匹配，状态栏 `col <name>: M/N`）。
  - `←`/`→` 或 `h`/`l` 在**单元格间左右移动**（状态栏显示 `row N · col <name>`）。
  - `v` 查看选中**单元格的完整值**（浮层，适合长文本/NULL）。
  - `c` **就地编辑选中单元格并写回**（`UPDATE`）：**仅当结果是单表预览时**可编辑（任意 SQL 结果只读）。输入新值 → 确认弹窗显示生成的 `UPDATE`（按整行原值匹配 `WHERE`，NULL 用 `IS NULL`）→ 执行后刷新并显示「updated N row(s)」。注：重复行会一起更新（值本相同）；设置为 NULL 暂未支持。
...
| `←`/`→` 或 `h`/`l` | 左右移动（Results 内在**单元格间**移动） |
| `Enter` | Connections=连接并展开数据库 / 选中数据库刷新 Schema；Schema=预览选中表 |
| `n` / `e` / `d` | 在 Connections 面板：新增 / 编辑 / 删除连接（密码存系统 keychain） |
| `/` | 检索（**随聚焦面板切换对象**）：Connections=**数据库名** / Schema=**表名** / Results=**全字段模糊** |
| `f` | Results：按**选中列**过滤 |
| `v` | Results：查看**选中单元格的完整值** |
| `c` | Results：**编辑选中单元格**并写回（仅单表预览；确认后 `UPDATE`） |
| `F5` | 执行 SQL 编辑器里的语句 |
